...
(* Decidability of [rigid], needed for deciding [wfe] of a box (we_box carries a
   [rigid 0 _ _] premise under the eager-box redesign).  Proved by the same
   bindings-fuel induction as [rbox_dec]: the structural cases (arr/rcd/mani/and/
   ands/all/box) recurse into subterms (smaller bindings), and the [rigid_cvar]
   case follows the [lookt] chain with [var_decr] exactly as [rbox_dec] does.
   Discharged below by the bindings-fuel induction. *)
...
(* B = tvar: for a non-box / non-var / non-mani left operand A, only [eq_eqr] can fire
   (the right var must resolve via [lookt]; an [inner] right var has no rule).  First
   decide well-formedness of the right var; if it is ill-formed, teq fails outright. *)
...
(* The [eq_boxl] route works for ANY right operand B (it is the only box rule that does
   not fix B's shape), so the [boxt] left case tries it first; on failure it returns the
   precise negation of [eq_boxl]'s premise bundle, leaving the B-specific route to refute. *)
...
(* Size-neutral padded recursion helpers.  Pushing a manifest into a key frame produces
   an [&s]/[&=]-padded, [tshift 0]-shifted subproblem whose [bindings] weight is UNCHANGED
   ([bindings_zero2]/[bindings_zero1]).  These wrap the strong IH so each decision branch
   can recurse on the padded subproblem with the ORIGINAL (unpadded) fuel.  Keeping the
   right operand [B] (resp. left operand [A]) abstract here means [tshift 0 B] never
   reduces, so the [bindings_zero2] rewrite always matches. *)
...
(* B = mani: for a left operand [A] that is not a tvar/box/mani, the only rule that can
   relate [A] to a right [mani B1 B2] is the (aligned, &s-padded) [eq_manir].  Decide its
   single &s/&=-padded subproblem; on its failure the inversion of any [teq] derivation
   leaves only that case, refuted by [Hnr].  (A = mani is excluded because then [eq_manil]
   also applies, and A = tvar/box because [eq_eql]/[eq_boxl] also apply; those left shapes
   are handled in their own decision branches.) *)
...
(* A = mani, B not tvar/box/mani: the only rule is the (aligned, &s/tshift-padded)
   [eq_manil].  Decide its padded subproblem; on failure inversion leaves only that case. *)
...
(* A = mani, B = mani: both [eq_manil] (left mani) and [eq_manir] (right mani) can fire.
   Try [eq_manil] first; on failure try [eq_manir]; refute both on the negative branch. *)
...
(* A = mani, B = box: both [eq_manil] and [eq_boxr] can fire.  Try [eq_manil] first; on
   failure try [eq_boxr] (its body subproblem + the box-wft side condition). *)
...
(* A = mani, B = tvar X: both [eq_manil] and [eq_eqr] can fire.  Try [eq_manil] first; on
   failure, if X is concrete ([lookt T2 X B']) try [eq_eqr] via the supplied resolver. *)
