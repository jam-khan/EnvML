...
Lemma is_box_tshift : forall X A, ~ is_box A -> ~ is_box (tshift X A).
Proof.
  introv H. destruct A; simpl; try solve [intro Hc; inverts Hc | exact H].
  destruct m; simpl; intro Hc; inverts Hc.
Qed.
#[export] Hint Resolve is_box_tshift : core.

Fixpoint num_of_abs (T: typ) : nat :=
  match T with
    | A & _  => num_of_abs A
    | A &s  => num_of_abs A + 1
    | A &= _ => num_of_abs A
...
Lemma wfe_lshape: forall T, wfe T -> lshape T.
Proof.
...
Lemma lookt_add_old: forall T i A, lookt T i A -> forall T2, lookt (T2 +++ T) i A.
Proof. intros; eapply lookt_app; eauto. Qed.
...
Lemma adde_teq_sp_r: forall T1 A B T2,
  teq T2 A B T1 -> forall C,
  wft (T1 & C) B ->
  teq T2 A B (T1 & C).
Proof.
  intros. eapply teq_sym. eapply teq_sym in H.
  eapply adde_teq_sp; eauto. 
Qed.

Lemma adde_wft_sp: forall A T1,
  wft T1 A -> forall B,
  wfe (T1 & B) ->
  wft (T1 & B) A.
Proof.
  intros. unfold wft in *. eapply add_evar_wft; eauto. econstructor. eapply add_evar_refl.
  unfold wft. eapply wfe_evar_eteq; eauto.
Qed.
...
Lemma ib_diag: forall X T T',
  insert_tvar X T T' -> insert_both X T T' T T'.
Proof.
  introv Hi. inductions Hi.
  - eapply ib_here2.
  - eapply ib_var_l. eapply ib_var_r. exact IHHi.
  - eapply ib_tvar. exact IHHi.
  - eapply ib_teq. exact IHHi.
Qed.
...
Lemma del_eq: forall T1 A B T2,
  teq T1 A B T2 -> forall T3,
  del_evar T1 T3 ->
  teq T3 A B T2.
Proof.
  introv Ht. inductions Ht; introv Hd; try solve [eauto];
    try solve [econstructor; eauto;
      solve [ match goal with |- teq _ _ _ _ =>
                eapply IHHt;
                solve [ eauto | eapply del_evar_app; eauto | eapply del_evar_pre; eauto ] end
            | eapply del_evar_wfe; try eapply Hd; eauto
            | eapply del_evar_wft; try eapply Hd; eauto
            | eapply del_check_inv; eauto
            | eapply del_lookt_inv; eauto ]].
  - (* eq_and *) econstructor; eauto.
    eapply IHHt2. eapply del_app; eauto.
    eapply teq_wft_left; eauto.
Qed.
...
Lemma is_box_dec: forall A, 
  is_box A \/ ~is_box A.
Proof.
  intros. destruct A; try solve [right; intros Hc; inverts Hc].
  - left. eauto.
Qed.
...
(* Splicing a context below preserves type-variable lookups (read from the top). *)


(* A self-contained env stays well-formed when spliced on top of any well-formed env. *)
...
Lemma teq_swap_outer: forall T2 T4 A B, wft T2 A -> wft T4 B ->
  forall T1 T3, teq (T1 +++ T2) A B (T3 +++ T4) ->
  forall T1' T3', wfe (T1' +++ T2) -> wfe (T3' +++ T4) ->
  teq (T1' +++ T2) A B (T3' +++ T4).
Proof.
  introv HwA HwB H. inductions H; introv Hw1 Hw2;
  try solve [eauto];
  (* structural / congruence via IH after inverting wft *)
  try solve [unfold wft in HwA, HwB; inverts HwA; inverts HwB; econstructor;
             first [eapply IHteq | eapply IHteq1 | eapply IHteq2];
             unfold wft in *; eauto];
  (* binder all: inner extends by &s on both sides *)
  try solve [unfold wft in HwA, HwB; inverts HwA; inverts HwB; eapply eq_all;
             repeat rewrite mcon_cons_st; eapply IHteq;
             solve [ unfold wft; eauto
                   | rewrite mcon_cons_st; reflexivity
                   | rewrite <- mcon_cons_st; econstructor; eauto ]];
  (* ands: inner unchanged, lshape side conditions *)
  try solve [unfold wft in HwA, HwB; inverts HwA; inverts HwB; eapply eq_ands;
             [ eapply IHteq; unfold wft in *; eauto | eauto | eauto ]];
  (* manil: left is mani -> invert HwA; padded with &= on left, &s on right (tshift 0 C) *)
  try solve [unfold wft in HwA, HwB; inverts HwA; eapply eq_manil;
             rewrite mcon_cons; rewrite mcon_cons_st; eapply IHteq;
             [ unfold wft; eassumption
             | eapply insert_tvar_wft;
                 [ eapply itv_here2
                 | eapply we_tvar; eapply wft_wfe; unfold wft in *; eassumption
                 | unfold wft; eassumption ]
             | apply mcon_cons | apply mcon_cons_st
             | eapply wfe_app; [ eassumption | eapply wfe_cut; eassumption ]
             | eapply wfe_app; [ eapply we_tvar; eapply wft_wfe; unfold wft in *; eassumption
                               | eapply wfe_cut; eassumption ] ]];
  (* manir: right is mani -> invert HwB; padded with &s on left (tshift 0 B), &= on right *)
  try solve [unfold wft in HwA, HwB; inverts HwB; eapply eq_manir;
             rewrite mcon_cons_st; rewrite mcon_cons; eapply IHteq;
             [ eapply insert_tvar_wft;
                 [ eapply itv_here2
                 | eapply we_tvar; eapply wfe_inv; unfold wft in *; eassumption
                 | unfold wft; eassumption ]
             | unfold wft; eassumption
             | apply mcon_cons_st | apply mcon_cons
             | eapply wfe_app; [ eapply we_tvar; eapply wfe_inv; unfold wft in *; eassumption
                               | eapply wfe_cut; eassumption ]
             | eapply wfe_app; [ eassumption | eapply wfe_cut; eassumption ] ]];
  (* tvar: same var X on both sides; swap the check-positions via check_swap_wft *)
  try solve [eapply eq_tvar;
    [ exact Hw1 | exact Hw2
    | eapply check_swap_wft; [ exact HwA | eassumption ]
    | eapply check_swap_wft; [ exact HwB | eassumption ] ]];
  (* and: congruence; second premise (body) extends inner by the lshape frame *)
  try solve [unfold wft in HwA, HwB; inverts HwA; inverts HwB; eapply eq_and;
    [ eapply IHteq1; solve [ unfold wft in *; eauto | reflexivity ]
    | eauto | eauto
    | repeat rewrite mapp_ass; eapply IHteq2;
      solve [ unfold wft in *; eauto
            | rewrite mapp_ass; reflexivity
            | reflexivity
            | eapply wfe_app; [ eapply wfe_inv; unfold wft in *; eauto | eapply wfe_cut; eauto ]
            | eapply wfe_cut; eauto ] ]];
  (* eql: left tvar; strip/swap the lookt binding via wft (handles both check & get) *)
  try solve [
    match goal with Hl: lookt (_ +++ T2) _ _ |- _ => forwards Hlt: lookt_strip_wft HwA Hl end;
    eapply eq_eql;
    [ eapply lookt_swap_wft; eassumption
    | eapply IHteq;
      solve [ eapply lookt_wft; [ eapply wft_wfe; eassumption | eassumption ]
            | unfold wft in *; eauto | reflexivity | eapply wfe_cut; eauto ] ]];
  (* eqr: mirror, binding in T4 *)
  try solve [
    match goal with Hl: lookt (_ +++ T4) _ _ |- _ => forwards Hlt: lookt_strip_wft HwB Hl end;
    eapply eq_eqr;
    [ eapply lookt_swap_wft; eassumption
    | eapply IHteq;
      solve [ eapply lookt_wft; [ eapply wft_wfe; eassumption | eassumption ]
            | unfold wft in *; eauto | reflexivity | eapply wfe_cut; eauto ] ]].
  (* box cases (NEW frame-local rules): bodies are frame-local; only the ambient
     wfe and the rigid / wft side conditions need the swapped prefix. *)
  - (* eq_boxl: A = boxt Tbody A0; body frame Tbody local, RHS ambient swaps via IH *)
    forwards (HwAbody & _ & HwfTb): boxt_wft_inv HwA.
    forwards Etf: mcon_top HwfTb.
    match goal with H' : teq ?Tb _ B _ |- _ => eapply eq_boxl with (T3 := Tb) end.
    + (* body: left frame Tbody = top +++ Tbody fixed; swap RHS T0+++T4 -> T3'+++T4 *)
      rewrite <- Etf. eapply IHteq;
        [ exact HwAbody | exact HwB
        | symmetry; exact Etf | reflexivity
        | rewrite Etf; exact HwfTb | exact Hw2 ].
    + (* box-wft on swapped left prefix: lift wft T2 (boxt Tbody A0) *)
      eapply wft_prepend; [ exact HwA | eapply wfe_cut; exact Hw1 ].
  - (* eq_boxr: B = boxt Tbody B0; body frame local, LHS ambient swaps via IH *)
    forwards (HwBbody & _ & HwfTb): boxt_wft_inv HwB.
    forwards Etf: mcon_top HwfTb.
    match goal with H' : teq _ A _ ?Tb |- _ => eapply eq_boxr with (T3 := Tb) end.
    + rewrite <- Etf. eapply IHteq;
        [ exact HwA | exact HwBbody | reflexivity | symmetry; exact Etf
        | exact Hw1 | rewrite Etf; exact HwfTb ].
    + eapply wft_prepend; [ exact HwB | eapply wfe_cut; exact Hw2 ].
Qed.
...
(* equated left-spines have the same number of stars (teq on lshapes only uses
   eq_top/eq_and/eq_ands, each star-count preserving). Needed for eq_and and for the
   depth-generalized rigid preservation below. *)
...
(* The inserted &s occupies one bound-variable level, so removing it DECREMENTS
   the rigid depth: the input depth is S d, the output depth d.  The insertion
   point X must lie within the binder window (X <= d). *)
...
(* ===== SELF-side reverse rigid shift on the RIGHT.  Removes an inserted tvar
   from the RIGHT environment, unshifting the RIGHT type, justified by the RIGHT
   type's OWN rigidity (in the unpadded right env).  Mirror of
   teq_shift_tvar_r_rigid_rev_aux, but the rigidity hypothesis is on the output
   (Bz, T2) pair instead of the input (A, T1) pair.  This is exactly the balanced
   metatheory the padded eq_manil/eq_boxr cases need: a rigid right operand can be
   strengthened off the &s padding even when the left operand is NOT rigid. *)
...
(* Public form: unpad the &s on the RIGHT env (and the tshift 0 on the right type)
   when the right type is rigid in the unpadded right env. *)
...
(* Two-sided REVERSE shift: inverse of shift_tvar_both.  Non-box cases mirror the
   forward proof; box cases reduce the rigid box BODY to a one-sided ambient
   reverse rigid shift (teq_shift_tvar_{r,l}_rigid_rev). *)
...
(* rigid_lookt: a rigid (tvar X) resolves (lookt) to a rigid type.  The rigid_cvar
   branch closes by lookt_det.  The rigid_bvar branch (X is one of the type's OWN
   d descended binders) is where a `&s`-forall own-var has no lookt (vacuous), while
   a `&=`-manifest own-var's stored value is rigid by the mani/and first premise --
   BUT lookt re-bases that value by `tshift 0`, which can push a forall-bound index
   from `<d` past `d`; closing that requires a rigid/tshift coherence on the manifest
   value.  Stated and used by the eq_eql/eq_eqr cases below. *)
(* rigid_lookt RELOCATED before teqd_trans. *)

(* dead_star_r_gen (RIGID-based): when the LEFT body A is rigid in L (so its only
   variable USES are forall-bound or concrete -- never a bare ambient abstract var),
   the right ambient star at the bottom of the context is unpaired, hence safely
   instantiated.  The invariant `Hinv` says every forall-bound left var (index < d)
   pairs at an abstract depth strictly inside the suffix p; combined with rigid this
   makes eq_tvar's bottom-star pairing impossible.  At a box body (eq_boxl) the fresh
   depth is 0, so Hinv is vacuous and the recursion is clean. *)
(* ===== one-sided rigid abstract-binder insert (the de-Bruijn core) ===== *)

(* inner_app_dec relocated earlier (before teqd_rigid_l). *)
(* align / align_and / rigid_lookt RELOCATED before teqd_trans. *)
...
Inductive exb : nat -> nat -> typ -> typ -> typ -> typ -> Prop :=
  | exb_base : forall T A,
      exb 0 1 (T &= A) ((T &= A) &s) (T &= A) ((T &s) &= tshift 0 A)
...
Lemma teq_mani_inv_r: forall T1 A B C T2,
  teq T1 C (mani A B) T2 ->
  teq (T1 &s) (tshift 0 C) B (T2 &= A).
Proof.
  introv Ht.
  eapply teq_sym. eapply teq_mani_inv_l. eapply teq_sym. exact Ht.
Qed.
