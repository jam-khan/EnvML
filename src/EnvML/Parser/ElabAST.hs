...
--   -- TySubstT: [x:=A] b
--   N.TyRcd keyvals -> error "TODO"
--   N.TyCtx env -> D.TyEnvt (elabTyEnv ctx env)
--   N.TyModule mt -> D.TyModule (elabModuleTyp ctx mt)

-- elabTyEnvE :: Ctx -> N.TyCtxE -> D.TyEnvE
...
--     N.TypE t -> D.TypE (elabTyp ctx t)
--     N.ModE m -> D.ModExpE (elabExp ctx m)
...
--   D.Clos (elabEnv ctx env) (elabExp (fmap fst a ++ ctx) e)
