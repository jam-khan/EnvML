...
-- Source (Named, Desugared)
-- Target (Nameless, Core EnvML)
import Debug.Trace (trace)
import qualified EnvML.Parser.AST as S
...
convExp ctx = \case
  ...
  S.Var x ->
    T.RProj (T.Var (lookupVar x ctx)) x -- TODO: verify
  S.Lam [(x, _arg)] body ->
...
convTyCtx outerCtx tyCtx =
  let tyCtxNames = extractTyCtxNamesForward tyCtx
      convertAt i e =
        let namesUpTo = reverse (take i tyCtxNames)
...
convEnv outerCtx env =
  let envNames = extractEnvNamesForward env
      ...
      convertAt i e =
        let namesUpTo = reverse (take i envNames)
            ctx = namesUpTo ++ outerCtx
         in convEnvE ctx e
...
convEnvE ctx = \case
  S.ExpEN n e -> T.ExpE (T.FEnv [T.ExpE $ T.Rec [(n, convExp ctx e)]])
...
convIntf outerCtx intf =
  let intfNames = extractIntfNamesForward intf
      convertAt i e =
        let namesUpTo = reverse (take i intfNames)
            ctx = namesUpTo ++ outerCtx
         in convIntfE ctx e
...
convModule ctx = \case
  ...
  S.MAnno m mt ->
    T.MAnno (convModule ctx m) (convModuleTyp ctx mt)
