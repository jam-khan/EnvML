...
theorem TypeSafeIndexLookup : ∀ (ST₁ ST₂ : EnvML.Typ) (n : Nat),
    EnvML.IndexLookup ST₁ n ST₂ →
    LambdaE.IndexLookup (EnvML.ElaborateType ST₁) n (EnvML.ElaborateType ST₂) := by
      intro ST₁ n ST₂ H;
      induction H with
      | zero A B =>
          apply LambdaE.IndexLookup.zero;
      | succ A B n C _ IH =>
          rw [EnvML.ElaborateType];
          apply LambdaE.IndexLookup.succ;
          apply IH;

theorem TypeSafeLabelExistence: ∀ (label : String) (ST : EnvML.Typ),
    EnvML.LabelIn label ST ↔
    LambdaE.LabelIn label (EnvML.ElaborateType ST) := by
      intro label ST;
      apply Iff.intro;
      · intro H;
        induction H with
      ...
      · intro H;
        induction ST with
        | rcd labelA T IH =>
            rw [EnvML.ElaborateType] at H;
            cases H
            apply EnvML.LabelIn.rcd
        | and A B IH1 IH2 =>
            rw [EnvML.ElaborateType] at H;
            cases H with
            | andl _ _ _ H1 =>
                apply EnvML.LabelIn.andl;
                apply IH1;
                apply H1;
            | andr _ _ _ H2 =>
                apply EnvML.LabelIn.andr;
                apply IH2;
                apply H2;
        | _ => cases H

theorem TypeSafeLabelNonExistence : ∀ (label : String) (ST : EnvML.Typ),
    ¬ EnvML.LabelIn label ST ↔
    ¬ LambdaE.LabelIn label (EnvML.ElaborateType ST) := by
      intro label ST;
      apply Iff.intro;
...
theorem TypeSafeRecordLookup : ∀ (ST₁ ST₂: EnvML.Typ) (label : String),
    EnvML.RecordLookup ST₁ label ST₂ →
    LambdaE.RecordLookup (EnvML.ElaborateType ST₁) label (EnvML.ElaborateType ST₂) := by
      intro ST₁ label ST₂ H;
      induction H with
      | zero label T =>
...
theorem TypePreservation : ∀ (EnvT ST : EnvML.Typ) (e : EnvML.Exp) (_ : LambdaE.Typ) (ce : LambdaE.Exp),
    EnvML.ElaborateExp EnvT e ST ce →
    LambdaE.HasType (EnvML.ElaborateType EnvT) ce (EnvML.ElaborateType ST) := by
      ...
      | sopen EnvT A B e₁ e₂ ce₁ ce₂ label H1 H2 H3 IH1 IH2 IH3 IH4 =>
          ...
          · apply LambdaE.HasType.rproj;
            · apply IH2;
            · apply TypeSafeRecordLookup;
              apply EnvML.RecordLookup.zero

theorem SlookupUniqueness : ∀ (EnvT T₁ T₂: EnvML.Typ) (n : Nat),
    EnvML.IndexLookup EnvT n T₁ →
    EnvML.IndexLookup EnvT n T₂ →
    T₁ = T₂ := by
...
theorem RLookupUniqueness : ∀ (RT T₁ T₂: EnvML.Typ) (label : String),
    EnvML.RecordLookup RT label T₁ →
    EnvML.RecordLookup RT label T₂ →
    T₁ = T₂ := by
...
theorem InferenceUniqueness : ∀ (EnvT T₁ T₂ : EnvML.Typ) (e : EnvML.Exp) (ce₁ ce₂ : LambdaE.Exp),
    EnvML.ElaborateExp EnvT e T₁ ce₁ →
    EnvML.ElaborateExp EnvT e T₂ ce₂ →
    T₁ = T₂ := by
      ...
      | app EnvT A B e₁ e₂ ce₁ ce₂ H1 H2 IH1 IH2 =>
          ...
          | app _ a' _ _ _ ce1' ce2' h1 h2 =>
            have hA : EnvML.Typ.arr A B = EnvML.Typ.arr a' T' := by apply IH1; apply h1;
            cases hA;
            rfl;
      | dmrg EnvT A B e₁ e₂ ce₁ ce₂ H1 H2 IH1 IH2 =>
          intro ce' T' H3;
      ...
      | modapp EnvT A B e₁ e₂ ce₁ ce₂ H1 H2 IH1 IH2 =>
          ...
          | modapp _ a' _ _ _ ce1' ce2' h1 h2 =>
            ...
            have hB : EnvML.Typ.sig A B = EnvML.Typ.sig a' T' := by apply IH1; apply h1;
            cases hB;
            rfl;
      | lrec EnvT A e ce label H IH =>
          intro ce' T' H2;
      ...
      | sopen EnvT A B e₁ e₂ ce₁ ce₂ label H1 H2 H3 IH1 IH2 IH3 IH4 =>
          ...
          | sopen _ a' b' _ _ ce1' ce2' label' h1 h2 h3 h4 =>
            have hA : EnvML.Typ.rcd label A = EnvML.Typ.rcd label' a' := by apply IH2; apply h2;
            cases hA;
            apply IH4;
            apply h4;

theorem ElaborationUniqueness : ∀ (EnvT T₁ T₂ : EnvML.Typ) (e : EnvML.Exp) (ce₁ ce₂ : LambdaE.Exp),
    EnvML.ElaborateExp EnvT e T₁ ce₁ →
    EnvML.ElaborateExp EnvT e T₂ ce₂ →
    ce₁ = ce₂ := by
      intro EnvT T₁ T₂ e ce₁ ce₂ H1 H2;
      ...
      | sopen EnvT A B e₁ e₂ ce₁ ce₂ label H1 H2 H3 IH1 IH2 IH3 IH4 =>
          ...
          cases H5 with
          | sopen _ a' b' _ _ ce1' ce2' label' h1 h2 h3 h4 =>
            have hA : EnvML.Typ.rcd label A = EnvML.Typ.rcd label' a' := by apply InferenceUniqueness EnvT (EnvML.Typ.rcd label A) (EnvML.Typ.rcd label' a') e₁ ce₁ ce1'; apply H2; apply h2;
            cases hA;
